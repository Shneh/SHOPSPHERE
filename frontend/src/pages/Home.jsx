  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '4rem' }}>
      ...
      <section className="glass-card" style={{ 
        textAlign: 'center', 
        padding: '4rem 2rem', 
        background: 'linear-gradient(135deg, rgba(99, 102, 241, 0.15) 0%, rgba(236, 72, 153, 0.05) 50%, rgba(11, 15, 25, 0.6) 100%)',
        ...
        <form onSubmit={handleSearch} style={{ maxWidth: '600px', margin: '0 auto', display: 'flex', gap: '0.75rem' }}>
          <input 
            type="text" 
            placeholder="Search products, brands, categories..." 
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            style={{ marginBottom: 0, padding: '0.9rem 1.2rem', borderRadius: '30px' }}
          />
          <button type="submit" className="btn-primary" style={{ padding: '0.9rem 2rem', borderRadius: '30px' }}>
            🔍 Search
          </button>
        </form>
      </section>
          ...
          <div className="product-grid">
            {[1, 2, 3, 4].map(n => (
              <div key={n} className="glass-card" style={{ padding: '1rem', height: '360px' }}>
                <div style={{ width: '100%', height: '200px', background: 'rgba(255,255,255,0.05)', borderRadius: '8px', animation: 'fadeIn 1s infinite alternate' }}></div>
                <div style={{ height: '20px', background: 'rgba(255,255,255,0.05)', borderRadius: '4px', marginTop: '1rem', width: '70%' }}></div>
                <div style={{ height: '15px', background: 'rgba(255,255,255,0.05)', borderRadius: '4px', marginTop: '0.5rem', width: '40%' }}></div>
                <div style={{ height: '40px', background: 'rgba(255,255,255,0.05)', borderRadius: '8px', marginTop: '1.5rem' }}></div>
              </div>
            ))}
          </div>
        ) : (
        ...
        <section>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '2rem' }}>
            {recommendations.map(p => (
              <div key={p.id} className="glass-card glow-hover" style={{ padding: '1rem', background: 'rgba(17, 24, 39, 0.4)' }}>
